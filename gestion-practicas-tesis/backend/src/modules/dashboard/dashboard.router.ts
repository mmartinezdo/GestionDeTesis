...
export const dashboardRouter = router({
  getEstadisticas: permissionProcedure('practicas:leer')
    .query(async ({ ctx }) => {
      ...
      if (role === 'ESTUDIANTE') {
        ...
        if (!estudiante) return { role, error: 'Estudiante no encontrado' };
        ...
        return {
          ...
          practicasPorEstado: Object.entries(
            estudiante.practicas.reduce((acc, p) => {
              acc[p.estado] = (acc[p.estado] || 0) + 1;
              return acc;
            }, {} as Record<string, number>)
          ).map(([estado, cantidad]) => ({ estado, cantidad })),
          tesisPorEstado: Object.entries(
            estudiante.tesis.reduce((acc, t) => {
      ...
      if (role === 'ASESOR') {
        ...
        if (!asesor) return { role, error: 'Asesor no encontrado' };
        ...
        return {
          ...
          practicasPorEstado: Object.entries(
            asesor.practicasAsignadas.reduce((acc, p) => {
              acc[p.estado] = (acc[p.estado] || 0) + 1;
              return acc;
            }, {} as Record<string, number>)
          ).map(([estado, cantidad]) => ({ estado, cantidad })),
          tesisPorEstado: Object.entries(
            asesor.tesisAsignadas.reduce((acc, t) => {
              acc[t.estado] = (acc[t.estado] || 0) + 1;
              return acc;
            }, {} as Record<string, number>)
          ).map(([estado, cantidad]) => ({ estado, cantidad })),
      ...
      // Default for other roles (Empresa, etc.)
      return { role, message: 'Dashboard limitado para este rol' };
    }),
  
  getTopEmpresas: permissionProcedure('practicas:leer')
    .query(async ({ ctx }) => {
      const role = ctx.user.rol;
      if (role !== 'ADMIN' && role !== 'COORDINADOR') {
        return []; // Top empresas solo para admins/coordinadores
      }
