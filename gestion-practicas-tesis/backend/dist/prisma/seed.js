...
async function main() {
    ...
    const practicasDef = [
        { titulo: 'Desarrollo Backend Node.js', desc: 'Implementación de microservicios y APIs REST.', req: 'Conocimientos de TypeScript, NestJS y PostgreSQL.', horas: 480, empIdx: 0, estIdx: 0, asIdx: 0, estado: client_1.EstadoPractica.EN_CURSO },
        { titulo: 'Asistente de Obra', desc: 'Supervisión de acabados y control de materiales.', req: 'Estudiante de últimos ciclos de Ing. Civil.', horas: 320, empIdx: 1, estIdx: 7, asIdx: 2, estado: client_1.EstadoPractica.ASIGNADO },
        { titulo: 'Analista de Procesos Salud', desc: 'Optimización de flujos de atención al paciente.', req: 'Manejo de BPMN y Excel avanzado.', horas: 400, empIdx: 2, estIdx: 1, asIdx: 1, estado: client_1.EstadoPractica.FINALIZADO },
        { titulo: 'Gestión de Talento Humano', desc: 'Apoyo en procesos de reclutamiento y selección.', req: 'Proactividad y buen trato.', horas: 240, empIdx: 3, estIdx: 4, asIdx: 1, estado: client_1.EstadoPractica.EN_CURSO },
        { titulo: 'Control de Calidad Agro', desc: 'Análisis de laboratorio en línea de producción.', req: 'Manejo de equipos de medición.', horas: 480, empIdx: 4, estIdx: 14, asIdx: 0, estado: client_1.EstadoPractica.POSTULADO },
        { titulo: 'Desarrollo Frontend React', desc: 'Maquetación de dashboards corporativos.', req: 'React, Tailwind CSS, Redux.', horas: 400, empIdx: 5, estIdx: 5, asIdx: 0, estado: client_1.EstadoPractica.EN_CURSO },
        { titulo: 'Analista de Riesgos', desc: 'Evaluación crediticia de MYPES.', req: 'Conocimientos de matemática financiera.', horas: 360, empIdx: 7, estIdx: 6, asIdx: 1, estado: client_1.EstadoPractica.RECHAZADO },
        { titulo: 'Supervisión de Seguridad Minera', desc: 'Apoyo en implementación de protocolos SSOMA.', req: 'Disponibilidad para trabajar en campamento.', horas: 600, empIdx: 9, estIdx: 12, asIdx: 2, estado: client_1.EstadoPractica.EN_CURSO },
    ];
    for (const p of practicasDef) {
        await prisma.practica.upsert({
            where: { id: practicasDef.indexOf(p) + 1 },
            update: {},
            create: {
                titulo: p.titulo,
                descripcion: p.desc,
    ...
    const tesisDef = [
        { titulo: 'Aplicación de Blockchain para la Trazabilidad de Productos Agrícolas', res: 'Propuesta de un sistema descentralizado...', estIdx: 0, asIdx: 0, estado: client_1.EstadoTesis.EN_DESARROLLO },
        { titulo: 'Impacto del Teletrabajo en la Productividad de las PYMES en Trujillo', res: 'Estudio descriptivo sobre la modalidad remota...', estIdx: 1, asIdx: 1, estado: client_1.EstadoTesis.REGISTRADA },
        { titulo: 'Análisis Estructural de Edificaciones con Disipadores de Energía', res: 'Simulación sísmica en terrenos arenosos...', estIdx: 7, asIdx: 2, estado: client_1.EstadoTesis.ASIGNADA },
        { titulo: 'Optimización de la Cadena de Suministro mediante Inteligencia Artificial', res: 'Uso de algoritmos genéticos para logística...', estIdx: 10, asIdx: 0, estado: client_1.EstadoTesis.EN_DESARROLLO },
        { titulo: 'Derechos Laborales en la Economía Gig: El caso de los Delivery en Perú', res: 'Análisis jurídico sobre la subordinación...', estIdx: 2, asIdx: 1, estado: client_1.EstadoTesis.SUSTENTADA },
        { titulo: 'Diseño de un Centro de Salud Sostenible con Materiales de la Región', res: 'Arquitectura bioclimática aplicada...', estIdx: 8, asIdx: 2, estado: client_1.EstadoTesis.EN_DESARROLLO },
        { titulo: 'Estrategias de Marketing Digital para el Sector Turismo Post-Pandemia', res: 'Plan de acción para reactivación económica...', estIdx: 6, asIdx: 1, estado: client_1.EstadoTesis.RECHAZADA },
        { titulo: 'Implementación de un Sistema ERP para el Control de Inventarios en Minería', res: 'Software integrado para gestión de repuestos...', estIdx: 5, asIdx: 0, estado: client_1.EstadoTesis.EN_DESARROLLO },
    ];
